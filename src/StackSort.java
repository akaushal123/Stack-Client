...
public class StackSort {
    ...
    private static int[] doStackSort(int data[]) {
	    ...
	    VectorStack<Integer> lowerValue = new VectorStack<Integer>();
	    VectorStack<Integer> upperValue = new VectorStack<Integer>();
	    
	    for(int a:data)
	    {
	    	//if upperstack is empty add value to upper stack
	    	...
	    	//check the input number and add it to stack
	    	if(a>=upperValue.peek())
	    	{
	    		//while value of upper stack is more than array value and stack is not empty add it to lower stack
	    		while(!upperValue.isEmpty() && a>upperValue.peek())
	    			lowerValue.push(upperValue.pop());
	    		//add value to upper stack
	    		upperValue.push(a);
	    	}
	    	else
	    	{
	    		//while value of lower stack is less than array value and stack is not empty add it to upper stack
	    		while(!lowerValue.isEmpty() && a<lowerValue.peek())
	    			upperValue.push(lowerValue.pop());
	    		//add value to lower stack
	    		lowerValue.push(a);
	    	}
	    }
    
	    //add remaining item to upper stack 
	    while(!lowerValue.isEmpty())
	    	upperValue.push(lowerValue.pop());
	    
	    int j = 0;
	    
	    //copy the data from upper stack to result array
	    while(!upperValue.isEmpty())
	        	result[j++] = upperValue.pop();
